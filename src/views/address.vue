        ...
        <div class="container">
          <div class="checkout-addr">
              ...
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" :class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId = item.addressId">
                    <dl>
                      <dt>{{item.userName}}</dt>
                      <dd class="address">{{item.addressName}}</dd>
                      <dd class="tel">{{item.tel}}</dd>
                    </dl>
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click.stop="delAddress(item.addressId)">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:;" class="addr-set-default-btn" v-if="!item.defaultAddress" @click="setDefault(item.addressId)"><i>Set default</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-if="item.defaultAddress">Default Adress</div>
                  </li>
                  <li class="addr-new">
            ...
            <div class="next-btn-wrap">
              <router-link :to="{path:'/orderConfirm',query:{'addressId':selectedAddrId}}" class="btn btn--m btn--red">Next</router-link>
            </div>
          </div>
        </div>
      </div>
...
const Bread = (resolve)=>{
	...
	export default{
          ...
          methods:{
          	...
          	expand(){ 
          	     if(this.limit == 3) {this.limit = this.addressList.length}
          	     else{
          	     	this.limit = 3
          	     }     		
